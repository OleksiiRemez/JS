...
const getLocalDate = (date, isSeconds = false, isISO = false) => {
    ...
    if (!isISO) {
        res = isSeconds
            ...
            : date.toLocaleString('uk-UA').replace(reg, '');
    } else {
...
// console.log(getLocalDate(new Date(1999999123456)));
...
let car = new Car(2997, 'Supra', 'Toyota', 2007);
let car2 = new Car(5200, 'GT500', 'Shelby', 1967);
...
const testPerformance = (iterations, func) => {
    let time = Date.now();
